        ...
        for (int maskRow = 0; maskRow < maskRC; maskRow++) {
            for (int maskCol = 0; maskCol < maskRC; maskCol++) {
                for (int dep = 0; dep < maskDepth; dep++) {
                    ...
                    sum += sharedImage[sharedIdx]*sharedMask[maskIdx];
                }   
            }
        }
...
int main() {
    ...
    // Store feature map
    storeConvolution("./test_output/convolution_results/cuda_our_mask_par.txt", output, outputRow, outputCol, output_channels);
